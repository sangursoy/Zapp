...
-- Add favorite subcategories (only those that exist in category_subcategories)
INSERT INTO public.user_favorite_subcategories (user_id, category, subcategory)
SELECT 'e8922fbb-d439-44cc-a74b-9cdd17bdc085', cs.category, cs.subcategory
FROM category_subcategories cs
WHERE (cs.category, cs.subcategory) IN (
  ...
  ('Entertainment'::topic_category, 'Video Games'),
  ('Education'::topic_category, 'Online Learning')
)
AND NOT EXISTS (
  SELECT 1 FROM public.user_favorite_subcategories 
  WHERE user_id = 'e8922fbb-d439-44cc-a74b-9cdd17bdc085'
  AND category = cs.category
  AND subcategory = cs.subcategory
);
